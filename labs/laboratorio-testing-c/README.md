...
> R:Se llama para iniciarlizar la cantidad en cero y que no tenga nada que genere un inconveniente a la hora de iniciar el conteo
...
> R: [FAIL] carrito_total(&c) => esperado 700, obtenido 350 
...
> R: en la linea 24, dice total += c->items[i].precio; 

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: multriplicar por la cantidad
...
> R:falla, devuelve  [FAIL] carrito_agregar(&c, p) => esperado 0, obtenido 1  (linea 71)
...
> R:es incorrecto, deberia ser "<" porque cuando cantidad llega a cuantro deja agregar un producto mas, provocando un overflow
...
> R:las lineas 29 y 30, no se ejecutaron porque ningun test llamo a la funcion
...
> R: el unitario testea una funcion de forma aislada y el de integracion testea el trabajo conjunto de varias funciones, el bug lo encontro primero el unitario.
...
> R: porque C no controla los limites de los arrays, generando que puede sobreescribir memoria.
...
> R: que lo hace de forma mas rapida y enfocada y reduce el fallo y tiempo de busqueda que provoca el ojo humano.
...
> R: por dio la casualidad que el resultado era ese porque era una sola unidad y el bug saltaba a la vista cuando subia la cantidad del producto
